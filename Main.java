...
public class Main extends JFrame {
    ...
    private PaymentManager paymentManager;  // ✅ CORRECTED

    public Main() {
        userManager = new UserManager();
        wageCalculator = new WageCalculator();
        projectManager = new ProjectManager();
        paymentManager = new PaymentManager();  // ✅ CORRECTED
        setupMainWindow();
    }
    ...
    private void setupMainWindow() {
        ...
        ModuleButton paymentsBtn = new ModuleButton("Payments & Escrow", "Milestone & Secure Payments", PURPLE_ACCENT, "🔒");  // ✅ CORRECTED SUBTITLE
    ...
    private static class CustomGradientPanel extends JPanel {
        ...
        public CustomGradientPanel(Color startColor, Color endColor, GradientType type) {
            this.startColor = startColor; this.endColor = endColor; this.type = type;
        }
        private static BufferedImage loadImage() {
            try { return ImageIO.read(CustomGradientPanel.class.getResource("/background.png")); }
            catch (Exception e) { System.err.println("Background image not found."); return null; }
        }
        @Override
        protected void paintComponent(Graphics g) {
            super.paintComponent(g);
            ...
            } else { g2d.setColor(new Color(34, 47, 62)); g2d.fillRect(0, 0, getWidth(), getHeight()); }
    ...
    private static class ModuleButton extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            g2d.setColor(shadowColor); g2d.fillRoundRect(shadowOffset, shadowOffset, w - shadowOffset, h - shadowOffset, arc, arc);
            g2d.setColor(isHovered ? hoverColor : baseColor); g2d.fillRoundRect(0, 0, w - shadowOffset, h - shadowOffset, arc, arc);
            g2d.setColor(accentColor); g2d.fillRoundRect(0, 0, 8, h - shadowOffset, arc, arc); g2d.fillRect(4, 0, 4, h - shadowOffset);
            g2d.setFont(new Font("Segoe UI", Font.BOLD, 36)); g2d.drawString(emoji, 30, 65);
            g2d.setColor(Color.WHITE); g2d.setFont(new Font("Segoe UI", Font.BOLD, 18)); g2d.drawString(title, 30, 105);
