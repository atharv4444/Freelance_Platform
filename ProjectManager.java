...
private final Color redAccent = new Color(231, 76, 60);
    ...
    public ProjectManager(DatabaseManager dbManager) {
...
bids = new ArrayList<>();
        ...
        initializeGUI();
        loadDataFromDatabase(); // Load from database instead of sample data
...
cardLayout.show(contentPanel, "STATUS");
    ...
    // --- Panel Creation Methods (unchanged) ---
...
refreshButton.addActionListener(e -> loadDataFromDatabase()); // Refresh from database
...
// Add Resume column to bids table
...
bidTable = new JTable(bidTableModel);
        ...
        // Add double-click to view resume
...
showAllButton.addActionListener(e -> loadDataFromDatabase()); // Refresh from database
...
viewResumeForBid(bidId);
    ...
    // --- UPDATED: Action & Data Methods with Database Integration ---
...
projects.clear();
        ...
        for (ProjectData projectData : dbProjects) {
            Project project = new Project(
                ...
                projectData.title,
                projectData.clientName, // Pass String
...
// File doesn't exist anymore
            ...
            Bid bid = new Bid(
                ...
                bidData.projectId,
                 ...
                 bidData.freelancerName, // Pass String
                bidData.amount,
...
refreshBidTable();
    ...
    // Table refresh methods (unchanged)
...
for (Bid b : bids) { 
            String prop = b.getProposal().length() > 30 ?
b.getProposal().substring(0, 30) + "..." : b.getProposal(); 
...
bidTableModel.addRow(new Object[]{b.getBidId(), b.getProjectId(), b.getFreelancerName(), "$" + b.getAmount(), b.getDays() + " days", resumeStatus, prop, b.getStatus()});
    ...
    // Validation and form methods (unchanged)
...
bidAmountField.setText(""); 
    ...
    // UPDATED: Stats methods using database
...
if (total > 0) successRateLabel.setText(((completed * 100) / total) + "%"); 
    ...
    private int getActiveProjectsCount() { return dbManager.getActiveProjects(); }
    private int getCompletedProjectsCount() { return dbManager.getCompletedProjects();
}
    private int getPendingBidsCount() { return dbManager.getPendingBids();
}
...
gbc.gridx = x; gbc.gridy = y; 
    ...
    private void styleTable(JTable table, Color headerColor) { table.setBackground(bgColor);
table.setForeground(textColor); table.setGridColor(inputBgColor); table.setRowHeight(30); table.setFont(new Font("Segoe UI", Font.PLAIN, 14)); table.setSelectionBackground(accentColor); table.setSelectionForeground(Color.WHITE); JTableHeader h = table.getTableHeader(); h.setBackground(headerColor); h.setForeground(Color.WHITE);
h.setFont(new Font("Segoe UI", Font.BOLD, 15)); table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() { public Component getTableCellRendererComponent(JTable t, Object v, boolean isSelected, boolean hasFocus, int r, int c) { final Component comp = super.getTableCellRendererComponent(t,v,isSelected,hasFocus,r,c); comp.setBackground(r % 2 == 0 ? bgColor : new Color(55, 63, 66)); comp.setForeground(isSelected ? Color.WHITE : textColor); return comp; } });
}
    private JLabel createStyledHeader(String text, Color color) { JLabel l = new JLabel(text, JLabel.CENTER);
l.setFont(new Font("Segoe UI", Font.BOLD, 28)); l.setForeground(color); l.setBorder(BorderFactory.createEmptyBorder(0,0,20,0)); return l; }
    private JLabel createStyledLabel(String text) { JLabel l = new JLabel(text);
l.setForeground(textColor); l.setFont(new Font("Segoe UI", Font.BOLD, 14)); return l; }
    private JTextField createStyledTextField(int columns) { JTextField tf = new JTextField(columns);
tf.setBackground(inputBgColor); tf.setForeground(textColor); tf.setCaretColor(accentColor); tf.setFont(new Font("Segoe UI", Font.PLAIN, 14)); tf.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(new Color(80,80,80)), BorderFactory.createEmptyBorder(5,5,5,5))); return tf;
}
    private JComboBox<String> createStyledComboBox(String[] items) { JComboBox<String> cb = new JComboBox<>(items); cb.setBackground(inputBgColor); cb.setForeground(textColor);
cb.setFont(new Font("Segoe UI", Font.PLAIN, 14)); return cb; }
    private JTextArea createStyledTextArea(int rows, int cols) { JTextArea ta = new JTextArea(rows, cols);
ta.setBackground(inputBgColor); ta.setForeground(textColor); ta.setCaretColor(accentColor); ta.setFont(new Font("Segoe UI", Font.PLAIN, 14)); ta.setLineWrap(true); ta.setWrapStyleWord(true); ta.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(new Color(80,80,80)), BorderFactory.createEmptyBorder(5,5,5,5))); return ta;
}
    private JButton createStyledButton(String text, Color color) { JButton b = new JButton(text);
b.setFont(new Font("Segoe UI", Font.BOLD, 14)); b.setBackground(color); b.setForeground(Color.WHITE); b.setFocusPainted(false); b.setCursor(new Cursor(Cursor.HAND_CURSOR)); b.setBorder(BorderFactory.createEmptyBorder(10, 20, 10, 20)); return b;
}
    private JPanel createStatCard(String title, JLabel valueLabel, Color color, String icon) { JPanel card = new GlassPanel();
card.setLayout(new BorderLayout()); card.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createMatteBorder(0, 5, 0, 0, color), BorderFactory.createEmptyBorder(20, 25, 20, 20))); JLabel titleLabel = new JLabel(title);
titleLabel.setFont(new Font("Segoe UI", Font.BOLD, 18)); titleLabel.setForeground(textColor); JLabel iconLabel = new JLabel(icon); iconLabel.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 40)); iconLabel.setHorizontalAlignment(JLabel.RIGHT);
JPanel bottomPanel = new JPanel(new BorderLayout()); bottomPanel.setOpaque(false); bottomPanel.add(valueLabel, BorderLayout.WEST); bottomPanel.add(iconLabel, BorderLayout.EAST); card.add(titleLabel, BorderLayout.NORTH); card.add(bottomPanel, BorderLayout.CENTER); return card;
}
    private JLabel createValueLabel(String text, Color color) { JLabel label = new JLabel(text);
label.setFont(new Font("Segoe UI", Font.BOLD, 48)); label.setForeground(color); return label; }
    
    // --- UI Inner Classes (unchanged) ---
    private class TexturedGradientPanel extends JPanel {
        private final BufferedImage texture;
public TexturedGradientPanel() { this.texture = loadImage(); }
        private BufferedImage loadImage() { try { return ImageIO.read(getClass().getResource("/background.png"));
} catch (Exception e) { System.err.println("BG image not found."); return null;
} }
        @Override protected void paintComponent(Graphics g) { super.paintComponent(g);
Graphics2D g2d = (Graphics2D) g.create(); g2d.setColor(bgColor); g2d.fillRect(0, 0, getWidth(), getHeight());
if (texture != null) { for (int y=0; y<getHeight(); y+=texture.getHeight()) for (int x=0; x<getWidth(); x+=texture.getWidth()) g2d.drawImage(texture, x, y, this);
} g2d.dispose(); }
    }
...
g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); g2d.setColor(new Color(44, 62, 80, 150)); g2d.fillRoundRect(0, 0, getWidth(), getHeight(), 20, 20); g2d.dispose();
}
    }
    private class NavButton extends JPanel {
        private boolean isHovered = false;
public NavButton(String text, String cardName) { setLayout(new FlowLayout(FlowLayout.LEFT, 20, 0)); setOpaque(false); setCursor(new Cursor(Cursor.HAND_CURSOR)); JLabel l = new JLabel(text); l.setForeground(textColor);
l.setFont(new Font("Segoe UI", Font.BOLD, 16)); setBorder(BorderFactory.createEmptyBorder(15, 20, 15, 0)); add(l);
addMouseListener(new MouseAdapter() { public void mouseClicked(MouseEvent e) { cardLayout.show(contentPanel, cardName); } public void mouseEntered(MouseEvent e) { isHovered = true; repaint(); } public void mouseExited(MouseEvent e) { isHovered = false; repaint(); } });
}
        @Override protected void paintComponent(Graphics g) { super.paintComponent(g);
if (isHovered) { Graphics2D g2d = (Graphics2D) g.create(); g2d.setColor(new Color(52, 73, 94)); g2d.fillRect(0, 0, getWidth(), getHeight()); g2d.dispose();
} }
    }

    // --- Data Model Inner Classes (FIXED) ---
    private static class Project {
        // Removed unnecessary 'clientId' field
        private String projectId, title, clientName, category, difficulty, status, description;
        private double budget; 
        private int deadline;

        // FIXED: Constructor now takes clientName (String) and includes description
        ...
        // Removed public int getClientId() {}
        public String getCategory() { return category;
        } public double getBudget() { return budget; } public String getDifficulty() { return difficulty;
        } public int getDeadline() { return deadline; } public String getStatus() { return status;
        } public String getDescription() { return description; } public void setStatus(String status) { this.status = status;
        }
    }

    private static class Bid {
        // Removed unnecessary 'freelancerId' field
        private String bidId, projectId, freelancerName, proposal, status;
        private double amount; 
        private int days;
        private File resumeFile;
        
        // FIXED: Constructor now takes freelancerName (String)
        ...
        // Removed public int getFreelancerId() {}
